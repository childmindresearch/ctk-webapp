...
export async function handle({ event, resolve }) {
  ...
  const userEmail =
    event.request.headers.get("X-MS-CLIENT-PRINCIPAL-NAME") || DEVELOPMENT_USER;
  logger.info({
    type: `User Request`,
...
function isUserAuthorized(
  ...
  if (
    !user.isAdmin &&
    ADMIN_ENDPOINT_PATHS.some((path) => event.request.url.match(path))
  ) {
    return false;
  }
  ...
  for (const endpoint of ADMIN_SPECIFIC_ENDPOINTS) {
    if (
      !user.isAdmin &&
      event.request.url.match(endpoint.path) &&
      event.request.method === endpoint.method
    ) {
      return false;
    }
  }
