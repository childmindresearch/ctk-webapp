<script lang="ts">
    ...
    function removeSubService(index: number) {
        subServices = subServices.filter((_: unknown, i: number) => i !== index)
        emitChange()
    }
    ...
    <Card class="border-l-4 border-l-secondary">
        ...
        <CardContent class="space-y-6">
                    ...
                    <Popover.Content class="w-full p-0" style="z-index: 50;">
                        <Command.Root>
                            ...
                            <Command.Group class="max-h-[300px] overflow-auto">
                                {#each serviceCompletions as serviceOption (serviceOption)}
                                    <Command.Item
                                        value={serviceOption}
                                        onSelect={() => handleServiceSelect(serviceOption)}
                                    >
                                        <Check
                                            class={cn("mr-2 h-4 w-4", service !== serviceOption && "text-transparent")}
                                        />
                                        {serviceOption}
                                    </Command.Item>
                                {/each}
            ...
            <div class="space-y-4">
                ...
                <div class="space-y-3">
                    {#each subServices as subService, subservice_index (subService)}
                                    ...
                                    <Popover.Content class="w-full p-0" style="z-index: 50;">
                                        <Command.Root>
                                            <Command.Input
                                                ...
                                                bind:value={subServices[subservice_index]}
                                            />
                                            <Command.Empty>No sub-service found.</Command.Empty>
                                            <Command.Group class="max-h-[300px] overflow-auto">
                                                {#each currentSubServiceAutoCompletions as subServiceOption (subServiceOption)}
                                                    <Command.Item
                                                        value={subServiceOption}
                                                        onSelect={() =>
                                                            handleSubServiceSelect(subservice_index, subServiceOption)}
                                                    >
                                                        <Check
                                                            class={cn(
                                                                "mr-2 h-4 w-4",
                                                                subServices[subservice_index] !== subServiceOption &&
                                                                    "text-transparent"
                                                            )}
                                                        />
                                                        {subServiceOption}
