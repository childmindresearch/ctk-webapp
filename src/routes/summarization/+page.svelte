  ...
  <div class="mb-8">
    ...
    <Alert.Root>
      <Alert.Description class="leading-relaxed">
        Upload a clinical report (.docx) to generate a summary. The clinical report should contain the 'clinical summary
        and impressions' and 'recommendations' sections, as we only send the paragraphs in between these.
      </Alert.Description>
    </Alert.Root>
  </div>
            ...
            <div class="space-y-4">
              ...
              {#if file}
                <div class="flex items-center gap-2 p-3 bg-muted rounded-md">
                  ...
                  <Button type="button" variant="ghost" size="icon" class="h-8 w-8" onclick={removeFile}>
                    <X class="w-4 h-4" />
                  </Button>
                </div>
              {/if}
