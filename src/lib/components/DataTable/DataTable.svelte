      ...
      <Table.Body>
        {#each paginated as row (row[idColumn])}
          <Table.Row class="hover:bg-muted/50">
            {#if showControls}
              <Table.Cell>
                <Controls onEdit={createRowCallback(onEdit, row.id)} onDelete={createRowCallback(onDelete, row.id)} />
              </Table.Cell>
            {/if}
        ...
        <div class="flex items-center gap-1">
          {#each getVisiblePages() as pageNum}
            {#if pageNum === 0 && getVisiblePages()[0] > 1}
              <Button variant="outline" size="sm" class="h-9 w-9" onclick={() => (currentPage = 0)}>1</Button>
            ...
            {#if pageNum === getVisiblePages().slice(-1)[0] && getVisiblePages().slice(-1)[0] < maxPages - 2}
              <span class="px-1 text-muted-foreground">...</span>
              <Button variant="outline" size="sm" class="h-9 w-9" onclick={() => (currentPage = maxPages - 1)}>
                {maxPages}
              </Button>
            {/if}
          {/each}
